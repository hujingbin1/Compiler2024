...
public:
    ...
    std::vector<Value *> vars_vector;

    /// @brief 函数内变量的映射表，提高查询的效率
    std::unordered_map<std::string, Value *> vars_map;
    int number;
};
...
public:
    ...
    // /// @brief 重写equals方法
    // /// @return 是否相等
    // bool & equals(std::string const & name);
    ...
    /// @brief 函数指令信息输出
    /// @param str 函数指令
    void toString(std::string & str, SymbolTable & symtab);
    ...
    void insertBlock(Var_Block * block);
    // void set_useParams();//新加
...
public:
    /// @brief 形式参数列表
    std::vector<FuncFormalParam> params; // TODO本身它是private的

    std::vector<Var_Block *> Blocks_vector;

    /// @brief 函数内变量的映射表，提高查询的效率
    std::unordered_map<int, Var_Block *> blockMap;

    Var_Block * currentBlock = nullptr;
